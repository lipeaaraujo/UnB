...
ps5=np.poly1d([1,6,-48,-106,687,-540])
ps4=np.poly1d([1,11,35,13,-60])
ps6=np.poly1d([1,14,43,-44,-329,-930,-675])
ps9=np.poly1d([1,-4,-10,-500,3383,-5292,-21990,119100,-52784,-184704])
...
def bairstow_zeros(ps,prt):    
    ...
    rts=np.zeros(n)*(1+1j)
    p_in=ps
    while (n>2): #repita enquanto ordem do polinômio > 2 
    ...
    if(n==2):  # última equação ordem 2
        ...
        if (prt):
         ...
         if (prt):  print("{:.5f}".format(rts[n-1]))
    rts = clean_array(rts)
    return (rts)
