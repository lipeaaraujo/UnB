...
A = np.array([[2,1,-3,6],[-1,3,2,-2],[-4,3,1,-3],[-8,3,2,1]])
b = np.array([-1,12,3,6])
...
def det_gauss(A,prt): # com pivotamento
   A= A.astype(float)
   [N,M]=A.shape;
   if(prt):
       print("Matriz A")
...
def EliminacaoGaussJordan(A,b,prt): # com pivotamento
   A= A.astype(float)
   b=b.astype(float)
   C=np.c_[A, b]
   [N,M]=C.shape;
...
def FatoracaoLU(A,prt): # com pivotamento
   A= A.astype(float)
   [N,M]=A.shape;
   L=np.eye(N)
   P=L.copy()
