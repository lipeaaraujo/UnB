...
def epsilon(n,m):
   eps = 1.0
   i=0
   while  (SomaFloat(1.0,eps,n,m,False) >1)and(i<m):
      eps /= 2
      soma = SomaFloat(1.0,eps,n,m,False)
      i=i+1
   eps *= 2
   print("Epsilon iterativo =",eps)
   print("Epsilon 2**-(m-1) =", 2**-(m-1))
   return eps
...
def SomaFloat(x1_in,x2_in,n,m,prt): 
   ...
   if(prt): #imprime entradas e matissa deslocada
        ...
        print("e2     = %s" % (str_out))
        if(desloc>=m):
          print("Underflow: deslocamento -> maior que %d" % (m-1)) 
     ...
     #  lista_uns = find(mant_s[1:m+1]==1)
       ...
       if (prt):
           ...
           if(lista_uns.size>0):
                ...
                print("Underflow: matissa nula após deslocamento <- maior que %d bits" % (m-1))
   if (trocar_sinais):
     soma[0]=1
